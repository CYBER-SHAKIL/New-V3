...
// Cache the API URL so we only fetch it once per boot, not on every call
let _cachedApiUrl = null;
const _FALLBACK_API = "https://mahmudx7-hinata.vercel.app";
...
const baseApiUrl = async () => {
  ...
  try {
    ...
    _cachedApiUrl = base.data.mahmud || _FALLBACK_API;
  } catch (_) {
    _cachedApiUrl = _FALLBACK_API;
  }
...
module.exports = { 
  ...
  onReply: async function ({ api, event, Reply }) {
      ...
      const filePath = path.join(__dirname, `album_tmp_${Date.now()}_${Math.random().toString(36).slice(2)}.mp4`);
