...
module.exports = {
	...
	onStart: async function ({ args, message, envGlobal, getLang }) {
		...
		try {
			...
			return message.reply(getLang("error", err.response?.data?.Message || err.message || "Request failed"));
		}
		...
		try {
			...
			return message.reply(`❌ Error: ${err.response?.data?.Message || err.message || "Request failed"}`);
		}
