// set bash title
process.stdout.write("\x1b]2;SHAKIL BOT V3\x1b\x5c");
...
const { login } = defaultRequire("xnil-ypb-fca");
let qr, Canvas;
try {
        ...
        qr = { readQrCode: async () => { throw new Error("Canvas not available for QR reading"); } };
        Canvas = null;
}
...
const titles = [
        ...
        [
                "S H A K I L  B O T  V3@" + currentVersion
        ],
        [
                "SHAKIL BOT"
        ]
...
let subTitle = `SHAKIL BOT V3@${currentVersion} — Facebook Messenger Bot by 𝗠𝗗 𝗦𝗛𝗔𝗞𝗜𝗟 𝗛𝗢𝗦𝗦𝗘𝗡`;
const subTitleArray = [];
...
const author = ("Customized by 𝗠𝗗 𝗦𝗛𝗔𝗞𝗜𝗟 𝗛𝗢𝗦𝗦𝗘𝗡 ♡ | Base: GoatBot V2 by NTKhang");
const srcUrl = ("FB: https://www.facebook.com/61588178231072");
const fakeRelease = ("ALL VERSIONS NOT RELEASED FROM THIS REPO ARE FAKE");
...
function filterKeysAppState(appState) {
        return appState.filter(item => ["c_user", "xs", "datr", "fr", "sb", "i_user", "dbln", "pas", "locale", "dpr", "vpd", "wl_cbv", "fbl_st", "ps_l", "ps_n"].includes(item.key));
}
...
async function startBot(loginWithEmail) {
        ...
        try {
                const tooOldVersion = (await axios.get("https://raw.githubusercontent.com/ntkhang03/Goat-Bot-V2-Storage/main/tooOldVersions.txt", { timeout: 5000 })).data || "0.0.0";
                ...
                log.warn("VERSION", "Could not check version (no internet or timeout). Continuing...");
        }
        /* { CHECK ORIGIN CODE } */
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        try {
                                // convert to promise
                                const item = await axios.get("https://raw.githubusercontent.com/ntkhang03/Goat-Bot-V2-Gban/master/gban.json", { timeout: 8000 });
                                dataGban = item.data;
                                ...
                                log.warn('GBAN', 'Could not reach GBAN server (network issue). Continuing without ban check.');
                                dataGban = {};
                        }
                        // ———————————————— NOTIFICATIONS ———————————————— //
                        let notification;
                        try {
                                const getNoti = await axios.get("https://raw.githubusercontent.com/ntkhang03/Goat-Bot-V2-Gban/master/notification.txt", { timeout: 8000 });
                                notification = getNoti.data;
                        }
                        catch (err) {
                                log.warn("NOTIFICATION", "Could not fetch notifications (network issue). Continuing.");
                                notification = "";
                        }
                        ...
                        logColor("#f5ab00", createLine("SHAKIL BOT"));
                        // —————————————————— SYSTEM INFO —————————————————— //
                        log.info("FCA ENGINE", `xnil-ypb-fca ✔ (Active)`);
                        log.info("BOT NAME", `SHAKIL BOT V3`);
                        log.info("COMMANDS", `${global.GoatBot.commands.size} loaded`);
                        log.info("EVENTS", `${global.GoatBot.eventCommands.size} loaded`);
                        log.info("UPTIME", `${convertTime(Date.now() - global.GoatBot.startTime)}`);
                        log.info("FB PAGE", `https://www.facebook.com/61588178231072`);
                        logColor("#f5ab00", character);
                        // ——————————— SHAKIL SIGNATURE ——————————— //
                        const shakilArt = [
                                "",
                                ...
                                gradient.cristal(" ███████║██║  ██║██║  ██║██║  ██╗██║███████╗"),
                                gradient.cristal(" ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝╚══════╝"),
                                gradient.vice  ("      ✦  𝗠𝗗 𝗦𝗛𝗔𝗞𝗜𝗟 𝗛𝗢𝗦𝗦𝗘𝗡  —  SHAKIL BOT V3  ✦"),
                                "",
                        ].join("\n");
                        console.log(shakilArt);
                        ...
                        // Cache handlerAction once — avoids re-calling the factory on every message
                        const cachedHandlerAction = require("../handler/handlerAction.js")(api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData);
                        // —————————————————— CALLBACK LISTEN —————————————————— //
