        ...
        return async function (event, message) {
                ...
                */
                ...
                async function onStart() {
                        ...
                        const noPrefixEnabled = config.noPrefix?.enable === true;
                        const usedPrefix = body.startsWith(prefix);
                        if (!usedPrefix && !(noPrefixEnabled && isAdminBot)) return;
                        const dateNow = Date.now();
                        const args = (usedPrefix ? body.slice(prefix.length) : body).trim().split(/ +/);
                        // ————————————  CHECK HAS COMMAND ——————————— //
                ...
                */
                async function onReaction() {
                        // ——————————————— REACT UNSEND ——————————————— //
                        const reactUnsend = config.reactUnsend;
                        if (reactUnsend?.enable === true && event.reaction !== undefined) {
                                const isAdminBot2 = (config.adminBot || []).includes(senderID);
                                const passAdmin = !reactUnsend.onlyAdmin || isAdminBot2;
                                const emojiMatch = !reactUnsend.emojis?.length || reactUnsend.emojis.includes(event.reaction);
                                if (passAdmin && emojiMatch) {
